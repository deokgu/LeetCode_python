...
list_tree = [[[] for _ in range(n+1)] for _ in range(n+1)]
list_is_tree = [] # 현재 나무가 위치한 x, y
for _ in range(m):
    x, y, z = map(int, input().split())
    list_tree[x][y].append(z)
    list_is_tree.append((x,y))

for _ in range(k):
    for x, y in list(set(list_is_tree)): # 봄
        remove_tree = []    
        list_tree[x][y].sort() # 작은 순서대로 
        for i in range(len(list_tree[x][y])):
            num_temp_food = list_now_food[x][y]
            need_food = list_tree[x][y][i]
